...
For a native WebMCP agent, use a browser build with WebMCP enabled; browser support and flags change while the API is experimental. The end-to-end tests use clean Playwright contexts and a simulated `document.modelContext` to exercise registration and execution. They do **not** prove an external agent will discover or use a particular tool.
...
- `npm test`: 157 DOM-free unit tests; `npm run test:e2e`: clean Chromium tests across ten varied page layouts plus both demos, multi-page navigation, and WebMCP registration. The browser tests simulate the WebMCP API. CI runs both.
